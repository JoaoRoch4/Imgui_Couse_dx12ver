...
ExampleDescriptorHeapAllocator::ExampleDescriptorHeapAllocator()
	: Heap(nullptr), HeapType(D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES),
	  HeapStartCpu(), HeapStartGpu(), HeapHandleIncrement(), FreeIndices() {}

ExampleDescriptorHeapAllocator::~ExampleDescriptorHeapAllocator() {}

void ExampleDescriptorHeapAllocator::Create(ID3D12Device *device,
...
void ExampleDescriptorHeapAllocator::Alloc(
  ...
  out_cpu_desc_handle->ptr =
	  HeapStartCpu.ptr + static_cast<SIZE_T>(idx * HeapHandleIncrement);
  out_gpu_desc_handle->ptr =
	  HeapStartGpu.ptr + static_cast<SIZE_T>(idx * HeapHandleIncrement);
}
