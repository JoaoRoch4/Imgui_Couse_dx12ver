...
std::string FontManager::OpenFontFileDialog(HWND hwnd) const {
    ...
    if (GetOpenFileNameW(&ofn) == TRUE) {
        ...
        int sizeNeeded = WideCharToMultiByte(
            CP_UTF8,        // Convert to UTF-8 encoding
            0,              // No special flags
...
const std::vector<ImFont*>& FontManager::GetLoadedFonts() const {
...
const std::map<std::string, ImFont*>& FontManager::GetFontMap() const {
    return m_fontMap;
}
...
std::vector<std::string> FontManager::SearchFontsInFolder(
...
std::vector<FontManager::FontInfo> FontManager::SearchFontsWithInfo(
    const std::string& folderPath, 
    bool recursive) const {
...
// @param recursive: Boolean flag - if true, searches all subdirectories
// @return: Map where key is font name and value is FontInfo with details
std::map<std::string,std::string> FontManager::SearchFontsInFolderAsMap(const std::string & folderPath, bool recursive) const
 {
