...
StyleManager::StyleManager()
	: m_styleConfig(),
	  m_configFilePath(L"style_config.json"),
...
void StyleManager::Open() {
	...
	if (!LoadConfiguration()) {
		...
		std::wcout << L"Style configuration loaded successfully from " << m_configFilePath << std::endl;
		ApplyStyleToImGui();
	}
}
...
bool StyleManager::LoadConfiguration() {
	try {
		...
		if (!file.is_open()) {
			std::wcerr << L"Failed to open style configuration file: " << m_configFilePath << std::endl;
			return false;
		}
...
bool StyleManager::SaveConfiguration() {
	try {
		...
		if (!file.is_open()) {
			std::wcerr << L"Failed to create/open style configuration file: " << m_configFilePath << std::endl;
			return false;
		}
...
void StyleManager::ApplyStyleToImGui() {
	...
	style.ItemInnerSpacing = ImVec2(m_styleConfig.ItemInnerSpacingX, m_styleConfig.ItemInnerSpacingY);
	...
	style.Colors[ImGuiCol_ScrollbarGrabHovered] = ColorRGBAToImVec4(m_styleConfig.ScrollbarGrabHovered);
	style.Colors[ImGuiCol_ScrollbarGrabActive] = ColorRGBAToImVec4(m_styleConfig.ScrollbarGrabActive);
	...
	// Handle both old and new Tab color enums
	...
	style.Colors[ImGuiCol_PlotHistogramHovered] = ColorRGBAToImVec4(m_styleConfig.PlotHistogramHovered);
	...
	// Handle NavHighlight with conditional compilation
	#ifdef ImGuiCol_NavHighlight
	style.Colors[ImGuiCol_NavHighlight] = ColorRGBAToImVec4(m_styleConfig.NavHighlight);
	#endif
	#ifdef ImGuiCol_NavWindowingHighlight
	style.Colors[ImGuiCol_NavWindowingHighlight] = ColorRGBAToImVec4(m_styleConfig.NavWindowingHighlight);
	#endif
...
void StyleManager::CaptureStyleFromImGui() {
	...
	// Capture Tab colors with conditional compilation for compatibility
	#ifdef ImGuiCol_TabSelected
	m_styleConfig.TabActive = ImVec4ToColorRGBA(style.Colors[ImGuiCol_TabSelected]);
	#else
	m_styleConfig.TabActive = ImVec4ToColorRGBA(style.Colors[ImGuiCol_TabActive]);
	#endif
	#ifdef ImGuiCol_TabDimmed
	m_styleConfig.TabUnfocused = ImVec4ToColorRGBA(style.Colors[ImGuiCol_TabDimmed]);
	#else
	m_styleConfig.TabUnfocused = ImVec4ToColorRGBA(style.Colors[ImGuiCol_TabUnfocused]);
	#endif
	#ifdef ImGuiCol_TabDimmedSelected
	m_styleConfig.TabUnfocusedActive = ImVec4ToColorRGBA(style.Colors[ImGuiCol_TabDimmedSelected]);
	#else
	m_styleConfig.TabUnfocusedActive = ImVec4ToColorRGBA(style.Colors[ImGuiCol_TabUnfocusedActive]);
	#endif
	...
	// Capture Nav colors with conditional compilation for compatibility
	#ifdef ImGuiCol_NavHighlight
	m_styleConfig.NavHighlight = ImVec4ToColorRGBA(style.Colors[ImGuiCol_NavHighlight]);
	#else
	m_styleConfig.NavHighlight = {1.0f, 1.0f, 1.0f, 1.0f}; // Default white
	#endif
	#ifdef ImGuiCol_NavWindowingHighlight
	m_styleConfig.NavWindowingHighlight = ImVec4ToColorRGBA(style.Colors[ImGuiCol_NavWindowingHighlight]);
	#else
	m_styleConfig.NavWindowingHighlight = {1.0f, 1.0f, 1.0f, 0.7f}; // Default white semi-transparent
	#endif
 ...
 */
const std::wstring& StyleManager::GetConfigFilePath() const {
 ...
 */
void StyleManager::SetConfigFilePath(const std::wstring& path) {
 ...
 */
StyleConfiguration& StyleManager::GetStyleConfig() {
 ...
 */
const StyleConfiguration& StyleManager::GetStyleConfig() const {
	return m_styleConfig;
}
